...
class SolverWrapper(object):
  ...
  def train_model(self, sess, max_iters):
    ...
    with sess.graph.as_default():
      ...
      self.optimizer = tf.train.MomentumOptimizer(0.0001, momentum)
      self.optimizer1 = tf.train.MomentumOptimizer(0.0001, momentum)
      self.optimizer2 = tf.train.MomentumOptimizer(0.0001, momentum)
      #train_op1 = tf.train.GradientDescentOptimizer(0.0001).minimize(loss, var_list=var1)
      ...
      if cfg.TRAIN.DOUBLE_BIAS:
          ...
          with tf.variable_scope('Gradient_Mult') as scope:
              for grad, var in gvs:
                  ...
                  if(var.name[0:12]==u'vgg_16/conv1' or var.name[0:12]==u'vgg_16/conv2'):
                      grads1.append((grad, var))
                  else:
                      grads2.append((grad, var))
          train_op1 = self.optimizer1.apply_gradients(grads1)
          train_op2 = self.optimizer2.apply_gradients(grads2)
          train_op = tf.group(train_op1, train_op2)
      ...
      #  grads1 = final_gvs[:len(var_list1)]  
      #  grads2 = final_gvs[len(var_list1):] 
      #  train_op1 = self.optimizer1.apply_gradients(zip(grads1, var_list1))  
      #  train_op2 = self.optimizer2.apply_gradients(zip(grads2, var_list2))
      #  train_op = tf.group(train_op1, train_op2)
      #  #train_op = self.optimizer.apply_gradients(final_gvs)
      #else:
